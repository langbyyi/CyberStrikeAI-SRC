...
## Reconnaissance (Network-Space Search)

Information-collection queries are proxied by the backend to FOFA / ZoomEye / Quake / Shodan, so API keys never reach the browser. Requires the `fofa:execute` permission.

- `POST /api/fofa/search`: network-space search.
- `POST /api/fofa/parse`: natural language to search syntax.

`/api/fofa/search` request body:

| Field | Meaning |
| --- | --- |
| `provider` | Engine: `fofa` (default), `zoomeye`, `quake`, `shodan`. |
| `query` | Engine-specific query syntax, required. |
| `size` | Result count cap (engine-dependent; FOFA up to 10000). |
| `page` | Page number, default 1. |
| `fields` | Result fields (engine-dependent, comma separated). |
| `full` | Return the full field set. |

`/api/fofa/parse` request body:

| Field | Meaning |
| --- | --- |
| `provider` | Target engine, as above. |
| `text` | Natural-language query, e.g. `hosts of Example Corp whose domain contains example`. |

The response matches the OpenAPI definitions in `/api-docs`: `total` is the total match count, `size` is the number actually returned, and `results` is the result array. The MCP equivalent is the `fofa_search` tool (Agent-callable, supports `provider` and `natural_language`; authorization `asset:read`).
