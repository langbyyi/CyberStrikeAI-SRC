...
## Network-Space Reconnaissance (FOFA / ZoomEye / Quake / Shodan)

Queries on the **Asset Management → Information Collection** page are proxied by the backend so API keys never reach the browser. All four engines accept `base_url` + `api_key`, or the environment variables `FOFA_API_KEY`, `ZOOMEYE_API_KEY`, `QUAKE_API_KEY`, `SHODAN_API_KEY`.
...
- When `fofa.base_url` is only a host/transit address (e.g. `https://fofoapi.com`), the backend appends the default API path automatically.
- Optional FOFA advanced fields: `email` (official account email; usually empty for third-party transit), `auth_mode` (`auto` / `key` / `bearer`), `bearer_token`, `verify_ssl`, `timeout_seconds`, `fallback_base_urls` (alternate hosts when the primary fails), and `endpoints` (per-endpoint credentials, takes precedence over the legacy fields above when non-empty). See the `fofa` section of `config.example.yaml` for details.
- Keys can also be set in **System Settings → Asset Management**.

## Web Search (Tavily)

The Agent `web_search` tool is backed by the Tavily API. `TAVILY_API_KEY` environment variable is also supported.

```yaml
websearch:
  api_key: "" # Tavily API Key (https://tavily.com free tier)
  base_url: "" # optional; defaults to https://api.tavily.com
  max_results: 5 # default result count (1-10)
  timeout_seconds: 30 # per-request timeout
  enabled: true # false disables the web search tool
```
