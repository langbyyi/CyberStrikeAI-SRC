...
## 网络空间测绘（FOFA / ZoomEye / Quake / Shodan）

「资产管理 → 信息收集」页的查询由后端代理，避免在前端暴露 Key。四引擎均支持 `base_url` + `api_key`，也可用环境变量 `FOFA_API_KEY`、`ZOOMEYE_API_KEY`、`QUAKE_API_KEY`、`SHODAN_API_KEY` 配置。

```yaml
fofa:
  base_url: https://fofa.info/api/v1/search/all # 可选，留空用默认；无需带路径，后端按引擎自动补齐
  api_key: ""
zoomeye:
  base_url: https://api.zoomeye.ai/v2/search # 可选
  api_key: ""
quake:
  base_url: https://quake.360.net/api/v3/search/quake_service # 可选
  api_key: ""
shodan:
  base_url: https://api.shodan.io # 可选
  api_key: ""
```

- `fofa.base_url` 只填域名/中转地址（如 `https://fofoapi.com`）时，后端自动补齐默认 API 路径。
- FOFA 高级字段（均可选）：`email`（官方账号邮箱，第三方中转通常留空）、`auth_mode`（`auto` / `key` / `bearer`）、`bearer_token`、`verify_ssl`、`timeout_seconds`、`fallback_base_urls`（主端点失败时的备选地址）、`endpoints`（逐端点独立鉴权，非空时优先于上方 legacy 字段）。完整说明见 `config.example.yaml` 的 `fofa` 段。
- 前端也可通过 **系统设置 → 资产管理** 填写各引擎 Key。

## 联网搜索（Tavily）

Agent 的 `web_search` 工具由 Tavily API 驱动，也支持 `TAVILY_API_KEY` 环境变量。

```yaml
websearch:
  api_key: "" # Tavily API Key（https://tavily.com 免费申请）
  base_url: "" # 可选，留空使用默认 https://api.tavily.com
  max_results: 5 # 默认返回条数（1-10）
  timeout_seconds: 30 # 单次搜索超时
  enabled: true # false 时不注册联网搜索工具
```
