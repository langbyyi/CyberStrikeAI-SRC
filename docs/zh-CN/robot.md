...
2. 左侧导航进入 **系统设置**  
3. 在左侧设置分类中点击 **机器人设置**（位于「C2」与「终端」之间）  
4. 按平台配置：
...
| **解绑** | 请求解除当前平台账号绑定（需发送 `确认`）；也可在 Web 端的绑定列表中撤销 |
| **身份** 或 **whoami** | 显示平台发送者 ID、鉴权模式、绑定状态及当前实际 RBAC 用户、角色和资源范围 |
| **帮助** | 显示命令帮助与说明 |
| **列表** 或 **对话列表** | 列出所有对话的标题与对话 ID |
| **切换 \<对话ID\>** 或 **继续 \<对话ID\>** | 指定对话 ID，后续消息在该对话中继续 |
| **新对话** 或 **清空** | 开启一个新对话，后续消息在新对话中 |
| **状态** 或 **status** | 汇总当前对话、角色、模式与绑定项目 |
| **任务** 或 **task** | 查看当前对话正在执行的任务状态 |
| **重命名 \<新标题\>** | 修改当前对话标题 |
| **删除 \<对话ID\>** | 删除指定对话（需发送 `确认`） |
| **停止** | 中断当前正在执行的任务 |
| **角色** 或 **角色列表** | 列出所有可用角色（渗透测试、CTF、Web 应用扫描等） |
| **角色 \<角色名\>** 或 **切换角色 \<角色名\>** | 切换当前使用的角色 |
| **模式** 或 **模式列表** | 列出对话模式（单代理/多代理等）与当前选择 |
| **模式 \<模式名\>** 或 **切换模式 \<模式名\>** | 切换当前对话模式 |
| **权限** 或 **permissions** | 查看当前实际 RBAC 用户的有效业务权限 |
| **诊断** 或 **doctor** | 检查机器人关键配置状态（需 `config:read`） |
| **确认** 或 **取消** | 处理删除对话、解绑等高风险操作的确认 |
| **项目** 或 **项目列表** | 列出所有项目（需启用项目功能） |
| **新建项目 \<名称\>** | 创建项目并绑定当前对话 |
| **绑定项目 \<ID或名称\>** | 将已有项目绑定到当前对话 |
| **解除项目** | 解除当前对话的项目绑定 |
| **漏洞提醒** | 查看/开关漏洞订阅提醒（`漏洞提醒 开启|关闭`、`漏洞提醒 仅严重|高危以上|中危以上`） |
...
3. **看程序日志**  
   - 启动后应看到：`钉钉 Stream 正在连接…`。  
   - 若出现 `钉钉 Stream 长连接断开（如睡眠/断网），将自动重连` 且带错误信息，多为 **Client ID / Client Secret 错误**或**开放平台未开通流式接入**。  
