...
func TestMCPWebSearchAuthorizationRequiresAssetRead(t *testing.T) {
	db, err := database.NewDB(filepath.Join(t.TempDir(), "mcp-websearch-authz.db"), zap.NewNop())
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	// 无 asset:read 权限 → 拒绝
	ctxNoPerm := authctx.WithPrincipal(context.Background(), authctx.NewPrincipal("u1", "user", database.RBACScopeAssigned, map[string]bool{"agent:execute": true}))
	if err := authorize(ctxNoPerm, builtin.ToolWebSearch, nil); err == nil {
		t.Fatal("web_search allowed without asset:read")
	}

	// 有 asset:read → 放行
	ctxPerm := authctx.WithPrincipal(context.Background(), authctx.NewPrincipal("u2", "user", database.RBACScopeAll, map[string]bool{"asset:read": true}))
	if err := authorize(ctxPerm, builtin.ToolWebSearch, nil); err != nil {
		t.Fatalf("web_search denied with asset:read: %v", err)
	}
}
