...
// ProbeEvidenceRow 是已完成工具执行的精简视图，用于校验待记录漏洞是否有
// 本会话真实可复现证据支撑（tool 名 + 参数 + 结果文本）。
type ProbeEvidenceRow struct {
	ToolName  string
	Arguments string
	Result    string
}

// CompletedProbeEvidenceForConversation 返回某会话最近一批已完成工具执行的
// (tool_name, arguments, result)。record_vulnerability 的可复现门用它在落库前
// 校验：本会话确实对该漏洞目标完成过真实工具探测——没有真实探测证据就不让记录，
// 防止编造或不可复现的漏洞条目进入漏洞库。
func (db *DB) CompletedProbeEvidenceForConversation(conversationID string, limit int) ([]ProbeEvidenceRow, error) {
	...
	query := `
		SELECT tool_name, arguments, COALESCE(result, '')
		FROM tool_executions
	...
	defer rows.Close()
	var out []ProbeEvidenceRow
	for rows.Next() {
		var r ProbeEvidenceRow
		if err := rows.Scan(&r.ToolName, &r.Arguments, &r.Result); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
